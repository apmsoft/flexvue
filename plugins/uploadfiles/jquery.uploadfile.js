...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
       ...
       this.startUpload = function () {
        ...
        this.createProgress = function (hosturl, sfilename,ofilename,filepath,filesize,file_type,image_size) {
            ...
            if(s.showFileCounter) 
                fileNameStr = obj.fileCounter + s.fileCounterStyle + ofilename;
            else fileNameStr = ofilename;
            
            
            if(s.showFileSize)
                fileNameStr += (filesize === parseInt(filesize, 10)) ? " ("+getSizeStr(filesize)+")" : " ("+filesize+")";
            ...
            if(s.showPreview)
            {
                pd.preview.attr('src',hosturl+filepath);
                pd.preview.show();
            }
            ...
            if(s.showDownload) {
                ...
                pd.download.click(function () {
                    var arr = [{sfilename:sfilename,ofilename:ofilename,fullname:filepath,file_type:file_type}];
                    if(s.downloadCallback) s.downloadCallback.call(obj, arr);
                });
            }
            if(s.showDelete)
            {
            ...
            //이미지 미리보기
            if(file_type=='image'){
                var preview = '<img data-width="'+image_size[0]+'" data-height="'+image_size[1]+'" data-filename="'+sfilename+'" class="ajax-file-upload-preview" src="'+ hosturl +filepath+'" style="width:'+s.previewWidth+'; height:'+s.previewHeight+';"> ';
                pd.filename.prepend(preview);
            }
        }
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj, file) {
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if(typeof(data[0]) != 'undefined'){
                        if(data[0].file_type=='image'){
                                    files['image_size']
                            var preview = '<img data-width="'+data[0].image_size[0]+'" data-height="'+data[0].image_size[1]+'" data-filename="'+data[0].sfilename+'" class="ajax-file-upload-preview" src="'+data[0].hosturl+data[0].fullname+'" style="width:'+s.previewWidth+'; height:'+s.previewHeight+';"> ';
                            pd.filename.prepend(preview);
                        }
                    }
                ...
                error: function (xhr, status, errMsg) {
                    console.log("uploadfile error xhr: " + JSON.stringify(xhr));
                    console.log("uploadfile error status: " + JSON.stringify(status));
                    console.log("uploadfile error errMsg: " + errMsg);
                    pd.cancel.remove();
                    progressQ.pop();
                    pd.abort.hide();
