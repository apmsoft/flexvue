define(['jquery'],
    function($) {
        ...
        var DaumPostMap = {
            target_id : '',
            map_y : 36.6423200323081,
            map_x : 127.489093909065,
            map : null,
            ...
            doPostCodeAddress : function(btn ,target_postcode, target_address,jubun_address)
            {
                var self = this;
                ...
                document.querySelector(btn).addEventListener('click',function()
                {
                    ...
                    let postcode_panel = Panel.onStart(postcode_panel_setting);

                    // make url
                    UrlUtil.pushUrlParams({
                        mode : 'postcode'
                    });
                    let url_param = Object.entries(UrlUtil._url_params).map(([key, val]) => `${key}=${encodeURIComponent(val)}`).join("&");
                    UrlUtil.pushState(UrlUtil._url_params, '', `${app.service_root_dir}/?${url_param}`);
                
                    DocAsyncTask.doGetContents(postcode_panel_setting, {}, {
                        success: function(tpl, resp) 
                        {
                            var postcode_layer = document.getElementById('bottomthird_docs_contents');
                            ...
                            daum.postcode.load(function(){
                                new daum.Postcode({
                                    oncomplete: function(data) {
                                        ...
                                        Panel.onStop(postcode_panel_setting.panel);
                                        // history.go(-2);
                                    },
                                    width : '100%',
                                    height : '500px',
